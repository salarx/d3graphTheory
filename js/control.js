...
function resizeContent() {
    ...
    if(!$("#lightbox").hasClass("hidden")) {
        let listHeight = $("#map-overlay").innerHeight() - $("#map-header").outerHeight() - 10;
        $("#map-list").css('max-height', '' + listHeight + 'px');
    }
...
function loadContent(query) {
    ...
    let currentChap = contentData[query];

    document.title = currentChap["content-title"] + " - Graph Coloring";
